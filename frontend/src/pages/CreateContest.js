...
const CreateContest = () => {
  ...
  // Replace durationMinutes with duration parts
        ...
        try {
          ...
          if (error.response && error.response.status === 401) {
            ...
            try {
              ...
              console.error(
                "Failed to refresh token on component load:",
                refreshError
              );
  ...
  const handleSaveContestDetails = async () => {
    try {
      ...
      // Validate at least one duration component is positive
      ...
      try {
        ...
        if (apiError.message === "Authentication error") {
          try {
            ...
            console.log(
              "New token after refresh (first 10 chars):",
              newToken ? newToken.substring(0, 10) + "..." : "No token found"
            );
            ...
            console.error(
              "Token refresh failed during contest creation:",
              refreshError
            );
            setErrorMessage(
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const problem = availableProblems.find(
          (p) => p.id === numericProblemId
        );
        if (problem) {
          setSelectedProblems([...selectedProblems, problem]);
          setAvailableProblems(
            availableProblems.filter((p) => p.id !== numericProblemId)
          );
        }
      }
    } catch (error) {
      ...
      if (error.response) {
        ...
        setErrorMessage(
          `Error adding problem: ${error.response.data.detail || error.message}`
        );
      } else {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (problem) {
          ...
          setSelectedProblems(
            selectedProblems.filter((p) => p.id !== numericProblemId)
          );
        }
      }
    } catch (error) {
        ...
        return (
          <Box sx={{ mt: 2, display: "flex", flexDirection: "column", gap: 2 }}>
            <TextField
              ...
              onChange={(e) =>
                setContestDetails({ ...contestDetails, name: e.target.value })
              }
              required
              fullWidth
            />
            ...
            {/* Replace duration minutes with HH:MM:SS inputs */}
            <Typography variant="subtitle1">Duration (HH:MM:SS)</Typography>
            ...
            <Button
              ...
              disabled={
                ...
                !contestDetails.description ||
                !contestDetails.starting_time ||
                ((parseInt(duration.hours, 10) || 0) <= 0 &&
                  (parseInt(duration.minutes, 10) || 0) <= 0 &&
                  (parseInt(duration.seconds, 10) || 0) <= 0)
              }
            >
